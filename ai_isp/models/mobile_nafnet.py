...
class MobileNAFNetW16(nn.Module):
    ...
    def __init__(self, config: MobileNAFNetConfig | None = None) -> None:
        ...
        if cfg.condition_dim != 24:
            raise ValueError("V3 Condition 维度必须为 24")
        widths = (cfg.base_width, cfg.base_width * 2, cfg.base_width * 4, cfg.base_width * 8)
    ...
    def forward(self, image: torch.Tensor, condition: torch.Tensor) -> torch.Tensor:
        ...
        feature = self.encoders[0](feature)
        encoder_outputs.append(feature)
        feature = self.downs[0](feature)
        feature = self.encoders[1](apply_film(feature, film_e1))
        encoder_outputs.append(feature)
        feature = self.downs[1](feature)
        feature = self.encoders[2](apply_film(feature, film_e2))
        encoder_outputs.append(feature)
        feature = self.downs[2](feature)
        feature = self.middle(apply_film(feature, film_middle))
    ...
    def topology_manifest(self) -> dict[str, object]:
        ...
        return {
            "model": "Tiny Conditional MobileNAFNet-W16 Dark Preview V3",
            ...
            "feature_channels": [self.config.base_width * (2 ** index) for index in range(4)],
            "condition_dim": 24,
